...
export function DatePickerExpiration({
  ...
  const years = Array.from(
    ...
    React.useEffect(() => {
      if(customerData[256]){
        const formattedDate = new Date(customerData[256])
  ...
  const handleMonthChange = (month: string) => {
    let newDate : Date;
    if(date == null){
      newDate = setMonth(new Date(), months.indexOf(month));
    }else{
  ...
  const handleYearChange = (year: string) => {
    let newDate : Date;
    if(date == null){
      newDate = setYear(new Date(), parseInt(year));
    }else{
...
const convertDate = (date : Date) => {
  ...
  return (
    <Popover open={open} onOpenChange={() => handleOpen(false)} >
      <label className='!font-[500] text-[#000]' >Expiration Date</label>
                  ...
                  <p className='font-medium  text-[14px]' >
                    {
                        ...
                        :
                        (
                            <span className='text-black' >   {customerData[256]}</span>
                        )
                    }
                  </p>                    
              </div>
